...
class JwtSpec extends UnitSpec {
  describe("Jwt") {
    ...
    val signatureMD5 = "eW2omY9kpadPg/M5ECg/SQ=="
    ...
    val signatureSHA512Alt = "3M6Q4hImCsJj+c7mnN6NFnmnoDSv5n1UF8WUNFQ/pvFaRYGNKiHDg/ZJhUYapTGzp6w+0fsXfp6WVHHGy++MJA=="
    val tokenSHA512Alt = Seq(header64SHA512Alt, claim64, signatureSHA512Alt).mkString(".")

    it("should encode") {
      assertResult(tokenMD5, "MD5") { Jwt.encode(headerMD5,claim,key,Option("HmacMD5")) }
      assertResult(tokenMD5Alt, "MD5 Alt") { Jwt.encode(headerMD5Alt,claim,key,Option("HMD5")) }
      assertResult(tokenSHA1, "SHA1") { Jwt.encode(headerSHA1,claim,key,Option("HmacSHA1")) }
      assertResult(tokenSHA1Alt, "SHA1 Alt") { Jwt.encode(headerSHA1Alt,claim,key,Option("HS1")) }
      assertResult(tokenSHA256, "SHA256") { Jwt.encode(headerSHA256,claim,key,Option("HmacSHA256")) }
      assertResult(tokenSHA256Alt, "SHA256 Alt") { Jwt.encode(headerSHA256Alt,claim,key,Option("HS256")) }
      assertResult(tokenSHA512, "SHA512") { Jwt.encode(headerSHA512,claim,key,Option("HmacSHA512")) }
      assertResult(tokenSHA512Alt, "SHA512 Alt") { Jwt.encode(headerSHA512Alt,claim,key,Option("HS512")) }
    }
