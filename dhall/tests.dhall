...
let excludesScala3 =
  List/map Text { project: Text, scala: Text } (\(p: Text) -> { project = p, scala = scala3 }) noScala3
...
let setup = [
  ...
  GithubActions.Step::{
    ...
    `with` = Some [
      ...
      { 
        mapKey = "key",
        mapValue = "\${{ runner.os }}-sbt-cache-v2-\${{ hashFiles('**/*.sbt') }}-\${{ hashFiles('project/build.properties') }}"
      }
    ]
  }
]
...
let mimaJob = {
  runs-on = GithubActions.types.RunsOn.ubuntu-latest,
  strategy = Some GithubActions.Strategy::{ matrix = toMap { project = allProjects } },
  steps = setup # [
    GithubActions.Step::{
...
let testsJob = {
  ...
  steps = setup # [
    GithubActions.Step::{
      ...
      run = Some "sbt ++\${{ matrix.scala }} \${{ matrix.project }}/test",
    },
    GithubActions.Step::{
      name = Some "Test with coverage",
      `if` = Some "startsWith(matrix.scala, '2.13')",
      run = Some "sbt ++\${{ matrix.scala }} coverage \${{ matrix.project }}/test coverageReport",
    },
    GithubActions.Step::{
...
in {
  ...
  on = GithubActions.On::{
    ...
    pull_request = Some GithubActions.PullRequest::{ branches = Some ["master"] },
  },
  jobs = {
    linting = lintingJob,
    mdoc = mdocJob,
