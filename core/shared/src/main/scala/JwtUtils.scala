...
object JwtUtils {
  ...
  /** Returns the expected signature byte array length (R + S parts) for the specified ECDSA
    ...
    *
    * @param algorithm
    *   The ECDSA algorithm. Must be supported and not {@@codenull} .
    * @return
  ...
  /** Transcodes the JCA ASN.1/DER-encoded signature into the concatenated R + S format expected by
    ...
    *
    * @param derSignature
    *   The ASN1./DER-encoded. Must not be {@@codenull} .
    * @param outputLength
  ...
  /** Transcodes the ECDSA JWS signature into ASN.1/DER format for use by the JCA verifier.
    *
    * @param signature
    *   The JWS signature, consisting of the concatenated R and S values. Must not be {@@codenull} .
    * @return
